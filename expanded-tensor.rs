...
mod tensor {
    ...
    use crate::op::{
        BackpropOp, BinaryOp, CmpOp, CustomOp1, CustomOp2, CustomOp3, Op, ReduceOp,
        UnaryOp,
    };
    ...
    impl Tensor {
        ...
        pub fn new<A: crate::device::NdArray>(
            array: A,
            device: &Device,
        ) -> Result<Self> {
        ...
        pub fn arange<D: crate::WithDType>(
            start: D,
            end: D,
            device: &Device,
        ) -> Result<Self> {
                    ...
                    crate::Error::Msg(
                            {
                                let res = ::alloc::fmt::format(
                                    format_args!("step cannot be zero"),
                                );
                                res
                            }
                                .into(),
        ...
        pub fn add(&self, rhs: &Self) -> Result<Self> {
            ...
            let storage = self
                ...
                .binary_impl::<
                    crate::op::Add,
                >(&*rhs.storage(), self.layout(), rhs.layout())?;
            let op = BackpropOp::new2(
                self,
                rhs,
                |t1, t2| Op::Binary(t1, t2, BinaryOp::Add),
            );
        ...
        pub fn mul(&self, rhs: &Self) -> Result<Self> {
            ...
            let op = BackpropOp::new2(
                self,
                rhs,
                |t1, t2| Op::Binary(t1, t2, BinaryOp::Mul),
            );
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn sub(&self, rhs: &Self) -> Result<Self> {
            let shape = self.same_shape_binary_op(rhs, "sub")?;
            let storage = self
                .storage()
                .binary_impl::<
                    crate::op::Sub,
            ...
            let op = BackpropOp::new2(
                self,
                rhs,
                |t1, t2| Op::Binary(t1, t2, BinaryOp::Sub),
            );
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn div(&self, rhs: &Self) -> Result<Self> {
            let shape = self.same_shape_binary_op(rhs, "div")?;
            let storage = self
                .storage()
                .binary_impl::<
                    crate::op::Div,
            ...
            let op = BackpropOp::new2(
                self,
                rhs,
                |t1, t2| Op::Binary(t1, t2, BinaryOp::Div),
            );
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn maximum<T: TensorOrScalar>(&self, rhs: T) -> Result<Self> {
            ...
            let shape = self.same_shape_binary_op(&rhs, "maximum")?;
            let storage = self
                .storage()
                .binary_impl::<
                    crate::op::Maximum,
            ...
            let op = BackpropOp::new2(
                self,
                &rhs,
                |t1, t2| Op::Binary(t1, t2, BinaryOp::Maximum),
            );
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn minimum<T: TensorOrScalar>(&self, rhs: T) -> Result<Self> {
            ...
            let shape = self.same_shape_binary_op(&rhs, "minimum")?;
            let storage = self
                .storage()
                .binary_impl::<
                    crate::op::Minimum,
            ...
            let op = BackpropOp::new2(
                self,
                &rhs,
                |t1, t2| Op::Binary(t1, t2, BinaryOp::Minimum),
            );
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn broadcast_add(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_add")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => lhs.broadcast_as(&shape)?.add(&rhs.broadcast_as(&shape)?),
                (false, true) => lhs.add(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.add(rhs),
                (false, false) => lhs.add(rhs),
            }
        }
        pub fn broadcast_mul(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_mul")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => lhs.broadcast_as(&shape)?.mul(&rhs.broadcast_as(&shape)?),
                (false, true) => lhs.mul(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.mul(rhs),
                (false, false) => lhs.mul(rhs),
            }
        }
        pub fn broadcast_sub(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_sub")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => lhs.broadcast_as(&shape)?.sub(&rhs.broadcast_as(&shape)?),
                (false, true) => lhs.sub(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.sub(rhs),
                (false, false) => lhs.sub(rhs),
            }
        }
        pub fn broadcast_div(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_div")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => lhs.broadcast_as(&shape)?.div(&rhs.broadcast_as(&shape)?),
                (false, true) => lhs.div(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.div(rhs),
                (false, false) => lhs.div(rhs),
            }
        }
        pub fn broadcast_maximum(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_maximum")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => {
                    lhs.broadcast_as(&shape)?.maximum(&rhs.broadcast_as(&shape)?)
                }
                (false, true) => lhs.maximum(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.maximum(rhs),
                (false, false) => lhs.maximum(rhs),
            }
        }
        pub fn broadcast_minimum(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_minimum")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => {
                    lhs.broadcast_as(&shape)?.minimum(&rhs.broadcast_as(&shape)?)
                }
                (false, true) => lhs.minimum(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.minimum(rhs),
                (false, false) => lhs.minimum(rhs),
            }
        }
        pub fn broadcast_eq(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_eq")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => lhs.broadcast_as(&shape)?.eq(&rhs.broadcast_as(&shape)?),
                (false, true) => lhs.eq(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.eq(rhs),
        ...
        pub fn broadcast_ne(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_ne")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => lhs.broadcast_as(&shape)?.ne(&rhs.broadcast_as(&shape)?),
                (false, true) => lhs.ne(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.ne(rhs),
                (false, false) => lhs.ne(rhs),
            }
        }
        pub fn broadcast_lt(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_lt")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => lhs.broadcast_as(&shape)?.lt(&rhs.broadcast_as(&shape)?),
                (false, true) => lhs.lt(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.lt(rhs),
                (false, false) => lhs.lt(rhs),
            }
        }
        pub fn broadcast_le(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_le")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => lhs.broadcast_as(&shape)?.le(&rhs.broadcast_as(&shape)?),
                (false, true) => lhs.le(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.le(rhs),
                (false, false) => lhs.le(rhs),
            }
        }
        pub fn broadcast_gt(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_gt")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => lhs.broadcast_as(&shape)?.gt(&rhs.broadcast_as(&shape)?),
                (false, true) => lhs.gt(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.gt(rhs),
                (false, false) => lhs.gt(rhs),
            }
        }
        pub fn broadcast_ge(&self, rhs: &Self) -> Result<Self> {
            ...
            let shape = lhs
                .shape()
                .broadcast_shape_binary_op(rhs.shape(), "broadcast_ge")?;
            let l_broadcast = shape != *lhs.shape();
            let r_broadcast = shape != *rhs.shape();
            match (l_broadcast, r_broadcast) {
                (true, true) => lhs.broadcast_as(&shape)?.ge(&rhs.broadcast_as(&shape)?),
                (false, true) => lhs.ge(&rhs.broadcast_as(&shape)?),
                (true, false) => lhs.broadcast_as(&shape)?.ge(rhs),
                (false, false) => lhs.ge(rhs),
            }
        }
        pub fn recip(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Recip>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Recip));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn neg(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Neg>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Neg));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn exp(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Exp>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Exp));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn log(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Log>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Log));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn sin(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Sin>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Sin));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn cos(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Cos>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Cos));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn tanh(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Tanh>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Tanh));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn abs(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Abs>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Abs));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn sqr(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Sqr>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Sqr));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn sqrt(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Sqrt>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Sqrt));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn gelu(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Gelu>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Gelu));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn gelu_erf(&self) -> Result<Self> {
            ...
            let storage = self
                .storage()
                .unary_impl::<crate::op::GeluErf>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::GeluErf));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn erf(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Erf>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Erf));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn relu(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Relu>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Relu));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn silu(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Silu>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Silu));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn ceil(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Ceil>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Ceil));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn floor(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Floor>(self.layout())?;
            let op = BackpropOp::new1(self, |s| Op::Unary(s, UnaryOp::Floor));
            Ok(from_storage(storage, shape.clone(), op, false))
        }
        pub fn round(&self) -> Result<Self> {
            let shape = self.shape();
            let storage = self.storage().unary_impl::<crate::op::Round>(self.layout())?;
        ...
        pub fn repeat<S: Into<Shape>>(&self, shape: S) -> Result<Tensor> {
            ...
            let mut inp = if self.rank() < repeats.len() {
                let shape = [
                    ::alloc::vec::from_elem(1, repeats.len() - self.rank()),
                    self.dims().to_vec(),
                ]
                    .concat();
                self.reshape(shape)?
            ...
            for (idx, &repeat) in repeats.iter().enumerate() {
                if repeat > 1 {
                    inp = Tensor::cat(&::alloc::vec::from_elem(&inp, repeat), idx)?;
                }
            }
            Ok(inp)
        }
        ...
        ///
        pub fn meshgrid<A: AsRef<Tensor>>(
            args: &[A],
            xy_indexing: bool,
        ) -> Result<Vec<Self>> {
            if args.len() <= 1 {
                Err(
                    Error::OpRequiresAtLeastTwoTensors {
                        op: "meshgrid",
                    }
                        .bt(),
                )?
            ...
            for idx in 0..args.len() {
                let mut ones = ::alloc::vec::from_elem(1usize, args.len());
                ones[idx] = shape[idx];
                ...
                dims => {
                    let dims = self
                        ...
                        .filter_map(|(dim_idx, &v)| {
                            if dims.contains(&dim_idx) { None } else { Some(v) }
                        })
                        .collect::<Vec<_>>();
                    self.reshape(dims)
                }
        ...
        fn reduce_impl<D: Dim>(
            ...
            if keepdim { Ok(res) } else { res.squeeze_dims(&[dim]) }
        }
        fn sum_impl<D: Dims>(&self, sum_dims: D, keepdim: bool) -> Result<Self> {
            ...
            let op = BackpropOp::new1(
                self,
                |a| Op::Reduce(a, ReduceOp::Sum, dims.to_vec()),
            );
            let sum = from_storage(storage, dims, op, false);
            if keepdim { Ok(sum) } else { sum.squeeze_dims(&sum_dims) }
        }
        ...
        pub fn clamp<T1: TensorOrScalar, T2: TensorOrScalar>(
            &self,
            min: T1,
            max: T2,
        ) -> Result<Self> {
        ...
        pub fn interpolate1d(&self, target_size: usize) -> Result<Self> {
            ...
            let op = BackpropOp::new1(
                self,
                |arg| Op::UpsampleNearest1D {
                    arg,
                    target_size,
                },
            );
            let storage = self.storage().upsample_nearest1d(self.layout(), target_size)?;
        ...
        pub fn avg_pool2d_with_stride<T: crate::ToUsize2>(
                    ...
                    crate::Error::Msg(
                            {
                                let res = ::alloc::fmt::format(
                                    format_args!(
                                        "kernel-size {0:?} is larger than the input size {1},{2}",
                                        kernel_size, h, w
                                    ),
                                );
                                res
            ...
            let storage = self.storage().avg_pool2d(self.layout(), kernel_size, stride)?;
        ...
        pub fn max_pool2d_with_stride<T: crate::ToUsize2>(
            ...
            let storage = self.storage().max_pool2d(self.layout(), kernel_size, stride)?;
            Ok(from_storage(storage, (n, c, h_out, w_out), op, false))
        }
        ...
        pub fn scatter_add<D: Dim>(
            ...
            let op = BackpropOp::new3(
                self,
                indexes,
                source,
                |t1, t2, t3| { Op::ScatterAdd(t1, t2, t3, dim) },
            );
            Ok(from_storage(storage, self.shape(), op, false))
        }
        /// Embeds the values of the `src` tensor into the `self` tensor on the specified dimension.
        pub fn slice_scatter<D: Dim>(
            &self,
            src: &Self,
            dim: D,
            start: usize,
        ...
        pub fn slice_scatter0(&self, src: &Self, start: usize) -> Result<Self> {
            ...
            let shape_ok = self
                ...
                .enumerate()
                .all(|(dim_idx, (&d1, &d2))| {
                    if 0 == dim_idx { d2 + start <= d1 } else { d1 == d2 }
                });
            if !shape_ok {
                Err(
                    Error::ShapeMismatchBinaryOp {
            ...
            src.storage().copy_strided_src(&mut storage, offset, src.layout())?;
            let op = BackpropOp::new2(
                self,
                src,
                |t1, t2| Op::SliceScatter0(t1, t2, start),
            );
            Ok(from_storage(storage, self.shape(), op, false))
        }
        ...
        pub fn index_add<D: Dim>(
            ...
            let op = BackpropOp::new3(
                ...
                indexes,
                source,
                |t1, t2, t3| { Op::IndexAdd(t1, t2, t3, dim) },
            );
            Ok(from_storage(storage, self.shape(), op, false))
        }
        ...
        pub fn index_select<D: Dim>(&self, indexes: &Self, dim: D) -> Result<Self> {
            ...
            let op = BackpropOp::new2(
                self,
                indexes,
                |t1, t2| Op::IndexSelect(t1, t2, dim),
            );
                ...
                match self.layout.contiguous_offsets() {
                    Some((o1, o2)) => {
                        ...
                        for idx_row in 0..dim1 {
                            rows.push(
                                data[idx_row * dim2..(idx_row + 1) * dim2].to_vec(),
                            )
                        }
                    }
                    None => {
                        ...
                        if !src_index.next().is_none() {
                            ::core::panicking::panic(
                                "assertion failed: src_index.next().is_none()",
                            )
                        }
                    }
                }
        ...
        pub fn to_vec3<S: crate::WithDType>(&self) -> Result<Vec<Vec<Vec<S>>>> {
            ...
            let from_cpu_storage = |cpu_storage: &crate::CpuStorage| {
                let data = S::cpu_storage_as_slice(cpu_storage)?;
                let mut top_rows = ::alloc::vec::Vec::new();
                match self.layout.contiguous_offsets() {
                    Some((o1, o2)) => {
                        let data = &data[o1..o2];
                        let dim23 = dim2 * dim3;
        ...
        fn flatten_<D1: Dim, D2: Dim>(
            ...
            if self.rank() == 0 {
                ...
                if start_dim < end_dim {
                    ...
                    dst_dims
                        .push(dims[start_dim..end_dim + 1].iter().product::<usize>());
        ...
        pub fn permute<D: Dims>(&self, dims: D) -> Result<Tensor> {
            ...
            let is_permutation = dims.len() == self.rank()
                && (0..dims.len()).all(|i| dims.contains(&i));
            if !is_permutation {
                return Err(
                    crate::Error::Msg(
                            {
                                let res = ::alloc::fmt::format(
                                    format_args!(
                                        "dimension mismatch in permute, tensor {0:?}, dims: {1:?}",
                                        self.dims(), dims
                                    ),
                                );
                                res
                    ...
                    _ => {
                        return Err(
                            crate::Error::Msg(
                                    {
                                        let res = ::alloc::fmt::format(
                                            format_args!("not implemented yet"),
                                        );
                                        res
                                    }
                                        .into(),
        ...
        /// # Ok::<(), candle_core::Error>(())
        /// ```
        pub fn reshape<S: crate::shape::ShapeWithOneHole>(
            &self,
            s: S,
        ) -> Result<Tensor> {
        ...
        pub fn stack<A: AsRef<Tensor>, D: Dim>(args: &[A], dim: D) -> Result<Self> {
            if args.is_empty() {
                Err(
                    Error::OpRequiresAtLeastOneTensor {
                        op: "stack",
                    }
                        .bt(),
                )?
        ...
        fn cat0<A: AsRef<Tensor>>(args: &[A]) -> Result<Self> {
            ...
            cat_dims[0] = 0;
            let mut offsets = <[_]>::into_vec(
                #[rustc_box]
                ::alloc::boxed::Box::new([0usize]),
            );
            for (arg_idx, arg) in args.iter().enumerate() {
                let arg = arg.as_ref();
                if arg.dtype() != dtype {
            ...
            if left == 0 && right == 0 {
                ...
                return Err(
                    crate::Error::Msg(
                            {
                                let res = ::alloc::fmt::format(
                                    format_args!("cannot use pad_with_same on an empty tensor"),
                                );
                                res
                            }
                                .into(),
        ...
        pub fn storage_and_layout(
            &self,
        ) -> (std::sync::RwLockReadGuard<'_, Storage>, &Layout) {
        ...
        pub fn apply_op3_no_bwd<C: CustomOp3>(
            &self,
            t2: &Self,
            t3: &Self,
            c: &C,
        ...
        pub fn apply_op1<C: 'static + CustomOp1 + Send + Sync>(
            &self,
            c: C,
        ) -> Result<Self> {
        ...
        pub fn apply_op2_arc(
            ...
            let op = BackpropOp::new2(
                self,
                rhs,
                |t1, t2| Op::CustomOp2(t1, t2, c.clone()),
            );
            Ok(from_storage(storage, shape, op, false))
        }
        pub fn apply_op2<C: 'static + CustomOp2 + Send + Sync>(
            &self,
            r: &Self,
            c: C,
        ) -> Result<Self> {
        ...
        pub fn apply_op3_arc(
            ...
            let op = BackpropOp::new3(
                ...
                t2,
                t3,
                |t1, t2, t3| { Op::CustomOp3(t1, t2, t3, c.clone()) },
            );
            Ok(from_storage(storage, shape, op, false))
        }
                    ...
                    crate::Error::Msg(
                            {
                                let res = ::alloc::fmt::format(
                                    format_args!(
                                        "axis {0} is too large, tensor rank {1}", axis, rank
                                    ),
                                );
                                res
                            }
                        ...
                        crate::Error::Msg(
                                {
                                    let res = ::alloc::fmt::format(
                                        format_args!(
                                            "axis {0} is too small, tensor rank {1}", axis, rank
                                        ),
                                    );
                                    res
                                }
        ...
        pub fn slice_assign<D: std::ops::RangeBounds<usize>>(
                    ...
                    crate::Error::Msg(
                            {
                                let res = ::alloc::fmt::format(
                                    format_args!(
                                        "slice-assign requires input with the same rank {0} <> {1}",
                                        self_dims.len(), src_dims.len()
                                    ),
                                );
                                res
                            }
                    ...
                    crate::Error::Msg(
                            {
                                let res = ::alloc::fmt::format(
                                    format_args!(
                                        "slice-assign requires input with the same rank as there are ranges {0} <> {1}",
                                        self_dims.len(), ranges.len()
                                    ),
                                );
                                res
                            }
            ...
            for (i, range) in ranges.iter().enumerate() {
                        ...
                        crate::Error::Msg(
                                {
                                    let res = ::alloc::fmt::format(
                                        format_args!(
                                            "slice-assign: empty range for dim {0}, {1} {2}", i,
                        ...
                        crate::Error::Msg(
                                {
                                    let res = ::alloc::fmt::format(
                                        format_args!(
                                            "slice-assign: upper bound is out of range for dim {1}, {2} {0}",
                                            self_dims[i], i, end_excluded
                                        ),
                                    );
                                    res
                                }
                        ...
                        crate::Error::Msg(
                                {
                                    let res = ::alloc::fmt::format(
                                        format_args!(
                                            "slice-assign: the range for dim {1} ({2}..{3}) does not match the size of src {0}",
                                            src_dims[i], i, start_included, end_excluded
                                        ),
                                    );
                                    res
                                }
                ...
                src = src
                    .pad_with_zeros(i, start_included, self_dims[i] - end_excluded)?;
                mask = mask
                    .pad_with_zeros(i, start_included, self_dims[i] - end_excluded)?;
            }
            mask.where_cond(&src, self)
        }
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Add<B> for Tensor {
        type Output = Result<Tensor>;
        fn add(self, rhs: B) -> Self::Output {
            Tensor::add(&self, rhs.borrow())
        }
    }
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Add<B> for &Tensor {
        type Output = Result<Tensor>;
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Add<Tensor> for Result<B> {
        type Output = Result<Tensor>;
        fn add(self, rhs: Tensor) -> Self::Output {
            Tensor::add(self?.borrow(), &rhs)
        }
    }
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Add<&Tensor> for Result<B> {
        type Output = Result<Tensor>;
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Add<Result<B>> for Tensor {
        type Output = Result<Tensor>;
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Sub<B> for Tensor {
        type Output = Result<Tensor>;
        fn sub(self, rhs: B) -> Self::Output {
            Tensor::sub(&self, rhs.borrow())
        }
    }
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Sub<B> for &Tensor {
        type Output = Result<Tensor>;
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Sub<Tensor> for Result<B> {
        type Output = Result<Tensor>;
        fn sub(self, rhs: Tensor) -> Self::Output {
            Tensor::sub(self?.borrow(), &rhs)
        }
    }
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Sub<&Tensor> for Result<B> {
        type Output = Result<Tensor>;
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Sub<Result<B>> for Tensor {
        type Output = Result<Tensor>;
    ...
    impl std::ops::Sub<f64> for Tensor {
        ...
        fn sub(self, rhs: f64) -> Self::Output {
            self.affine((|_| 1.)(rhs), (|v: f64| -v)(rhs))
        }
    }
    impl std::ops::Sub<f64> for &Tensor {
        type Output = Result<Tensor>;
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Mul<B> for Tensor {
        type Output = Result<Tensor>;
        fn mul(self, rhs: B) -> Self::Output {
            Tensor::mul(&self, rhs.borrow())
        }
    }
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Mul<B> for &Tensor {
        type Output = Result<Tensor>;
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Mul<Tensor> for Result<B> {
        type Output = Result<Tensor>;
        fn mul(self, rhs: Tensor) -> Self::Output {
            Tensor::mul(self?.borrow(), &rhs)
        }
    }
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Mul<&Tensor> for Result<B> {
        type Output = Result<Tensor>;
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Mul<Result<B>> for Tensor {
        type Output = Result<Tensor>;
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Div<B> for Tensor {
        type Output = Result<Tensor>;
        fn div(self, rhs: B) -> Self::Output {
            Tensor::div(&self, rhs.borrow())
        }
    }
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Div<B> for &Tensor {
        type Output = Result<Tensor>;
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Div<Tensor> for Result<B> {
        type Output = Result<Tensor>;
        fn div(self, rhs: Tensor) -> Self::Output {
            Tensor::div(self?.borrow(), &rhs)
        }
    }
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Div<&Tensor> for Result<B> {
        type Output = Result<Tensor>;
    ...
    impl<B: std::borrow::Borrow<Tensor>> std::ops::Div<Result<B>> for Tensor {
        type Output = Result<Tensor>;
